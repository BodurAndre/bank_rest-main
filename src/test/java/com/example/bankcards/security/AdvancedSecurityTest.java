...
/**
 * Расширенные тесты безопасности для проверки ролей и доступа
 */
@SpringBootTest
@AutoConfigureWebMvc
@ActiveProfiles("test")
class AdvancedSecurityTest {
    ...
    void setUp() {
        ...
        // Setup other card
        otherCard = new BankCard();
        otherCard.setId(2L);
        otherCard.setMaskedNumber("**** **** **** 5678");
        otherCard.setBalance(BigDecimal.valueOf(500.00));
        otherCard.setStatus(BankCard.Status.ACTIVE);
        otherCard.setExpiryDate(LocalDate.now().plusYears(2));
        otherCard.setOwner(otherUser);
    }

    @Test
    void testUnauthenticatedAccess_AllProtectedEndpoints_ShouldReturn401() throws Exception {
    ...
    void testUserAccess_OwnResources_ShouldReturn200() throws Exception {
        when(bankCardService.findByOwner(any(User.class), any()))
                .thenReturn(org.springframework.data.domain.Page.empty());
        when(transferService.getTransferHistory(any(User.class), any()))
                .thenReturn(org.springframework.data.domain.Page.empty());

        // User can access their own cards
        mockMvc.perform(get("/api/cards"))
                .andExpect(status().isOk());

        // User can access their own transfers
        mockMvc.perform(get("/api/transfers"))
                .andExpect(status().isOk());
    }
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_AdminResources_ShouldReturn403() throws Exception {
    ...
    void testAdminAccess_AllResources_ShouldReturn200() throws Exception {
    ...
    void testUserAccess_OtherUserCard_ShouldReturn403() throws Exception {
        when(bankCardRepository.findById(2L)).thenReturn(Optional.of(otherCard));

        // User cannot access other user's card
        mockMvc.perform(put("/api/cards/2/block")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"reason\": \"Test reason\"}"))
                .andExpect(status().isForbidden());

        mockMvc.perform(put("/api/cards/2/activate"))
                .andExpect(status().isForbidden());

        mockMvc.perform(delete("/api/cards/2"))
                .andExpect(status().isForbidden());
    }
    ...
    void testUserAccess_OwnCard_ShouldReturn200() throws Exception {
        ...
        when(bankCardService.activateCard(anyLong()))
                .thenReturn(BankCardDto.fromEntity(testCard));

        // User can access their own card
        mockMvc.perform(put("/api/cards/1/block")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"reason\": \"Test reason\"}"))
                .andExpect(status().isOk());
    ...
    void testUserTransfer_OtherUserCard_ShouldReturn403() throws Exception {
        TransferRequest transferRequest = new TransferRequest();
        transferRequest.setFromCardId(1L);
        transferRequest.setToCardId(2L); // Card owned by another user
        transferRequest.setAmount(BigDecimal.valueOf(100.0));
        transferRequest.setDescription("Test transfer");
    ...
    void testUserAccess_ExportEndpoints_ShouldReturn403() throws Exception {
        ...
        mockMvc.perform(get("/admin/export/transfers/csv"))
                .andExpect(status().isForbidden());
    }
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_AdminAuditLogs_ShouldReturn403() throws Exception {
    ...
    @Test
    @WithMockUser(roles = "ADMIN")
    void testAdminAccess_AuditLogs_ShouldReturn200() throws Exception {
        mockMvc.perform(get("/admin/audit-logs"))
                .andExpect(status().isOk());
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_OwnAuditLogs_ShouldReturn200() throws Exception {
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_InvalidCardId_ShouldReturn404() throws Exception {
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_InvalidUserId_ShouldReturn404() throws Exception {
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_MalformedRequest_ShouldReturn400() throws Exception {
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_CSRFProtection() throws Exception {
    ...
    @Test
    @WithMockUser(roles = "USER")
    void testUserAccess_WithCSRF_ShouldWork() throws Exception {
        TransferRequest transferRequest = new TransferRequest();
        transferRequest.setFromCardId(1L);
        transferRequest.setToCardId(2L);
        transferRequest.setAmount(BigDecimal.valueOf(100.0));
